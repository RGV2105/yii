...
CREATE TABLE `asientos` (
  `id_asiento_reservado` int NOT NULL AUTO_INCREMENT,
  ...
  UNIQUE KEY `unique_asiento_funcion` (`id_reserva`,`fila`,`numero`),
  CONSTRAINT `asientos_ibfk_1` FOREIGN KEY (`id_reserva`) REFERENCES `reservas` (`id_reserva`) ON DELETE CASCADE
...
CREATE TABLE `funciones` (
  ...
  CONSTRAINT `funciones_ibfk_1` FOREIGN KEY (`id_pelicula`) REFERENCES `peliculas` (`id_pelicula`) ON DELETE CASCADE,
  CONSTRAINT `funciones_ibfk_2` FOREIGN KEY (`id_sala`) REFERENCES `salas` (`id_sala`) ON DELETE CASCADE
...
CREATE TABLE `pagos` (
  ...
  `metodo_pago` enum('tarjeta','efectivo','transferencia','app') NOT NULL,
  `fecha_pago` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `estado` enum('pendiente','completado','reembolsado','fallido') DEFAULT 'pendiente',
...
CREATE TABLE `peliculas` (
  ...
  `portada` varchar(255) DEFAULT NULL COMMENT 'Ruta o URL de la imagen de portada',
...
CREATE TABLE `reservas` (
  ...
  KEY `idx_codigo_confirmacion` (`codigo_confirmacion`),
  CONSTRAINT `reservas_ibfk_1` FOREIGN KEY (`id_cliente`) REFERENCES `clientes` (`id_cliente`) ON DELETE CASCADE,
  CONSTRAINT `reservas_ibfk_2` FOREIGN KEY (`id_funcion`) REFERENCES `funciones` (`id_funcion`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `salas` (
  ...
  `tipo_sala` enum('2D','3D','4DX','VIP') NOT NULL,
...
-- Dump completed on 2025-06-03 16:47:56
